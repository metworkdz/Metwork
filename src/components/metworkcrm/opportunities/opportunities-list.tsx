...
export function OpportunitiesList() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(`/api/metworkcrm/opportunities?${params.toString()}`);
  ...
  async function changeStage(row: OpportunityRow, nextStage: string) {
    setRows((prev) => prev.map((r) => (r.id === row.id ? { ...r, stage: nextStage } : r)));
    const res = await fetch(`/api/metworkcrm/opportunities/${row.id}`, {
      method: 'PATCH',
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <OpportunityFormDialog
          onSaved={(id) => router.push(`/metworkcrm/sales/${id}`)}
          trigger={
            <CrmButton size="sm" className="ms-auto">
              <Plus className="size-4" aria-hidden /> Nouvelle opportunité
            </CrmButton>
          }
        />
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-neutral-200 bg-white">
        <Table>
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={4} className="py-8 text-center text-sm text-neutral-400">Chargement…</TableCell>
              </TableRow>
            ) : rows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} className="py-8 text-center text-sm text-neutral-400">Aucune opportunité trouvée.</TableCell>
              </TableRow>
            ) : (
              rows.map((opp) => (
                <TableRow key={opp.id}>
                  <TableCell
                    ...
                    onClick={() => router.push(`/metworkcrm/sales/${opp.id}`)}
                  >
                    {opp.title}
                  </TableCell>
                  <TableCell>{OPPORTUNITY_TYPE_LABELS[opp.type] ?? opp.type}</TableCell>
                  <TableCell>{opp.amount != null ? `${opp.amount.toLocaleString('fr-FR')} DZD` : '—'}</TableCell>
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <select
                      value={opp.stage}
                      onChange={(e) => changeStage(opp, e.target.value)}
