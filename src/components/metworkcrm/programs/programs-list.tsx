...
export function ProgramsList() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(`/api/metworkcrm/programs?${params.toString()}`);
  ...
  async function changeStage(row: ProgramRow, next: string) {
    setRows((prev) => prev.map((r) => (r.id === row.id ? { ...r, stage: next } : r)));
    const res = await fetch(`/api/metworkcrm/programs/${row.id}`, {
      method: 'PATCH',
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <ProgramFormDialog
          onSaved={(id) => router.push(`/metworkcrm/programs/${id}`)}
          trigger={<CrmButton size="sm" className="ms-auto"><Plus className="size-4" aria-hidden /> Nouveau programme</CrmButton>}
        />
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={4} className="py-8 text-center text-sm text-neutral-400">Aucun programme trouvé.</TableCell></TableRow>
            ) : (
              rows.map((p) => (
                <TableRow key={p.id}>
                  <TableCell
                    className="cursor-pointer font-medium text-[var(--crm-black)]"
                    onClick={() => router.push(`/metworkcrm/programs/${p.id}`)}
                  >
                    {p.title}
                  </TableCell>
                  <TableCell>{PROGRAM_TYPE_LABELS[p.type] ?? p.type}</TableCell>
                  <TableCell>{p.startDate ?? '—'}{p.endDate ? ` → ${p.endDate}` : ''}</TableCell>
