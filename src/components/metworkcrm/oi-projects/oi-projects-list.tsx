...
export function OiProjectsList() {
  const router = useRouter();
  const [rows, setRows] = useState<OiProjectRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const res = await fetch(`/api/metworkcrm/oi-projects?${params.toString()}`);
  ...
  async function changeStage(row: OiProjectRow, next: string) {
    setRows((prev) => prev.map((r) => (r.id === row.id ? { ...r, stage: next } : r)));
    const res = await fetch(`/api/metworkcrm/oi-projects/${row.id}`, {
      method: 'PATCH',
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <OiProjectFormDialog
          onSaved={(id) => router.push(`/metworkcrm/open-innovation/${id}`)}
          trigger={<CrmButton size="sm" className="ms-auto"><Plus className="size-4" aria-hidden /> Nouveau projet</CrmButton>}
        />
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-neutral-200 bg-white">
        <Table>
          ...
          <TableBody>
            {loading ? (
              <TableRow><TableCell colSpan={3} className="py-8 text-center text-sm text-neutral-400">Chargement…</TableCell></TableRow>
            ) : rows.length === 0 ? (
              <TableRow><TableCell colSpan={3} className="py-8 text-center text-sm text-neutral-400">Aucun projet trouvé.</TableCell></TableRow>
            ) : (
              rows.map((p) => (
                <TableRow key={p.id}>
                  <TableCell
                    className="cursor-pointer font-medium text-[var(--crm-black)]"
                    onClick={() => router.push(`/metworkcrm/open-innovation/${p.id}`)}
                  >
                    {p.title}
                  </TableCell>
                  <TableCell>{p.budget != null ? `${p.budget.toLocaleString('fr-FR')} DZD` : '—'}</TableCell>
