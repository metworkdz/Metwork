...
import {
  Avatar, BrandButton, EmptyBlock, ErrorBanner, Field,
...
export function BookingsSection({ mentorId }: { mentorId: string }) {
  ...
  return (
    <section>
      ...
      <div className="mb-4 flex gap-1.5 overflow-x-auto rounded-md bg-muted p-1">
        {filters.map(({ key, label }) => {
          ...
          return (
            <button
              ...
              className={cn(
                'shrink-0 whitespace-nowrap rounded-md px-3 py-2 text-xs font-medium transition-colors',
                active
            ...
            <RowButton
              ...
              trailing={
                <div className="flex shrink-0 flex-col items-end gap-1">
                  ...
                  <span className="text-xs tabular-nums text-muted-foreground">{fmtDZD(b.amountCharged)}</span>
                </div>
              }
            />
          ))
  ...
  return (
      ...
      footer={
        view === 'detail' && isManageable ? (
          ...
          <button
            ...
            className="inline-flex min-h-11 w-full items-center justify-center rounded-md bg-red-600 px-4 text-sm font-semibold text-white transition-colors hover:bg-red-700 disabled:opacity-50"
          >
            {t('cancelConfirm')}
          </button>
        ) : undefined
    ...
    >
      {view === 'detail' && (
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-lg font-semibold tabular-nums text-foreground">{fmtDZD(booking.amountCharged)}</span>
          </div>
          ...
          {booking.meetingMode === 'ONLINE' && booking.meetingSource === 'auto' && (
            <div className="space-y-2 rounded-md border p-3 border-border bg-muted">
              <div className="flex items-center justify-between gap-2">
                <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t('autoGeneratedLabel')}</p>
              ...
              {booking.meetingLink && (
                <a href={booking.meetingLink} target="_blank" rel="noopener noreferrer" dir="ltr"
                  className="block truncate text-xs underline text-primary">
                  {booking.meetingLink}
                </a>
              )}
              {booking.zoomStartUrl && (
                <a href={booking.zoomStartUrl} target="_blank" rel="noopener noreferrer" dir="ltr"
                  className="inline-flex items-center gap-1.5 text-xs font-medium underline text-primary">
          ...
          {booking.meetingMode === 'OFFLINE' && booking.meetingAddress && (
            <div className="space-y-1 rounded-md border p-3 border-border bg-muted">
              <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{t('addressLabel')}</p>
              <p className="text-sm text-foreground">{booking.meetingAddress}</p>
              {booking.meetingMapsLink && (
                <a href={booking.meetingMapsLink} target="_blank" rel="noopener noreferrer"
                  dir="ltr" className="inline-block text-xs underline text-primary">
                  Google Maps
                </a>
              )}
            </div>
          ...
          {/* Client PII */}
          <div className="space-y-2 rounded-md border p-3 border-border bg-muted">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t('clientHeading')}</p>
            ...
            {booking.message && (
              <div className="pt-1">
                <p className="text-[11px] font-medium text-muted-foreground">{t('projectLabel')}</p>
                <p className="mt-0.5 whitespace-pre-wrap text-sm text-foreground">{booking.message}</p>
              </div>
            )}
          </div>

          {(booking.rescheduleCount ?? 0) > 0 && (
            <p className="text-xs text-muted-foreground">{t('rescheduledTimes', { count: booking.rescheduleCount ?? 0 })}</p>
          )}

          {/* Secondary actions */}
          {isManageable && (
            <div className="flex flex-wrap gap-2 border-t pt-3 border-border">
        ...
        <div className="space-y-4">
          ...
          {mode === 'ONLINE' ? (
            ...
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground">{t('confirmOfflineHeading')}</p>
      ...
      {view === 'cancel' && (
        <div className="space-y-3">
          {back}
          <p className="text-sm text-foreground">{t('cancelDescription')}</p>
...
function Fact({ icon, value }: { icon: React.ReactNode; value: string }) {
  return (
    <div className="flex items-center gap-2 rounded-md border px-3 py-2 text-xs text-foreground border-border bg-muted">
...
function Contact({ icon, label, value, dir }: { icon: React.ReactNode; label: string; value: string; dir?: 'ltr' | 'rtl' }) {
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <span dir={dir} className="text-foreground">{value || '—'}</span>
    </div>
  );
}
