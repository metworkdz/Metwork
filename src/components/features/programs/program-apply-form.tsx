...
export function ProgramApplyForm({ program, status, onSuccess }: ProgramApplyFormProps) {
  ...
  // Wallet balance — only matters for paid programs.
  ...
  const insufficient = !isFree && isAuthed && balance != null && balance < program.price;
      ...
      <div className="rounded-lg border border-border bg-muted/40 p-4 text-sm">
        ...
        {!isFree && (
          <div className="mt-2 flex items-center justify-between border-t border-border/60 pt-2 text-base font-semibold">
            <span>Total</span>
            <span className="tabular-nums">{formatCurrency(program.price, locale)}</span>
          </div>
        )}
      </div>
      ...
      {!isAuthed ? (
        ...
        <div className="space-y-2">
          <Button asChild className="w-full" size="lg" variant="outline">
            <Link href="/dashboard/entrepreneur/wallet">
              Top up to {formatCurrency(program.price, locale)}
            </Link>
          </Button>
        ...
        <Button type="submit" size="lg" className="w-full" loading={submitting}>
          {submitting
            ...
            : isFree
            ? 'Apply for free'
            : `Apply — ${formatCurrency(program.price, locale)}`}
        </Button>
      )}
    </form>
  );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <dl className="mt-3 grid grid-cols-2 gap-2 text-xs text-emerald-900">
            ...
            <div>
              <dt className="text-emerald-700">{paid ? 'Paid' : 'Application fee'}</dt>
