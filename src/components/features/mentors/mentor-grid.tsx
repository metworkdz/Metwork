...
interface MentorGridProps {
  ...
  /**
   * AI-match results, ranked best first. Matched mentors are pinned to the
   * top of the (still category-filtered) list — matching narrows attention
   * within what's already browsable, it never bypasses an active category
   * filter or hides anyone else.
   */
  matches?: { mentorId: string; reason: string }[] | null;
}
...
export function MentorGrid({
  ...
  const ordered = useMemo(() => {
    if (!matches || matches.length === 0) return filtered;
    const matchedIds = new Set(matches.map((m) => m.mentorId));
    const byId = new Map(filtered.map((m) => [m.id, m]));
    const pinned = matches
      .map((m) => byId.get(m.mentorId))
      .filter((m): m is Mentor => !!m);
    const rest = filtered.filter((m) => !matchedIds.has(m.id));
    return [...pinned, ...rest];
  }, [filtered, matches]);
      ...
      {ordered.map((m) => (
        <li key={m.id}>
          <MentorCard
            ...
            isRecommended={matchedIds.has(m.id)}
          />
        </li>
      ))}
    </ul>
