 ...
 * Admin-only mentor image upload. Uploads to Cloudinary when configured,
 * falls back to public/uploads on disk in dev.
 */
...
export async function POST(req: NextRequest) {
  ...
  if (!isSupportedMime(file.type)) return jsonError(415, 'UNSUPPORTED_MEDIA_TYPE', 'Image must be jpg, png, webp, gif, or avif');

  const buffer = Buffer.from(await file.arrayBuffer());

  if (isConfigured()) {
    try {
      const url = await uploadBuffer(buffer, { folder: 'metwork/mentors' });
      return json({ url, size: file.size }, { status: 201 });
    } catch (err) {
      console.error('[mentors/upload] Cloudinary error:', err);
  ...
  try {
    ...
    console.error('[mentors/upload] Filesystem fallback error:', err);
    return jsonError(500, 'UPLOAD_FAILED', 'File write failed. Configure CLOUDINARY_* env vars for production.');
  }
  return json({ url: `/uploads/mentors/${filename}`, filename, size: file.size }, { status: 201 });
}
