 ...
 * withdrawal request. Admin only. Delegates to the central withdrawal service
 * (which reuses the mentor-ledger resolver — no parallel withdrawal system).
 *
 * APPROVED: the manual external transfer (bank / CCP / cheque) is done; the
 *           escrow hold is completed. Idempotent on retry. Optional receiptUrl.
...
export async function PATCH(
  ...
  if (!result.replayed) {
    ...
    void appendAuditLog({
      ...
      details: {
        amount: r.amount,
        mentorId: 'mentorId' in r ? r.mentorId : undefined,
