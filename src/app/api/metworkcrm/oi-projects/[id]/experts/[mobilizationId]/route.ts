/**
 * PATCH  /api/metworkcrm/oi-projects/:id/experts/:mobilizationId — update role/status
 * DELETE /api/metworkcrm/oi-projects/:id/experts/:mobilizationId — remove the mobilization
 */
...
import { removeOiExpert, updateOiExpert } from '@/server/metworkcrm/services/oi-projects';
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    await updateOiExpert(mobilizationId, parsed.data);
    return json({ ok: true });
  } catch (err) {
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  try {
    await removeOiExpert(mobilizationId);
    return noContent();
  } catch (err) {
    return crmErrorResponse(err);
  }
