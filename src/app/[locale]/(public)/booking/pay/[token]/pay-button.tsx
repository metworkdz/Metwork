...
/**
 * Kicks off the hosted-checkout for a card-settled booking (SPACE / PROGRAM /
 * EVENT). Calls the public settlement route with { action: 'init' } and sends
 * the browser to the provider URL it returns. No card fields ever live in our
 * UI. Mirrors the guest-consultation pay button.
 */
export function BookingPayButton({ token }: { token: string }) {
  const t = useTranslations('pages.bookingPay');
  ...
  async function onPay() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/bookings/pay/${encodeURIComponent(token)}`, {
        method: 'POST',
