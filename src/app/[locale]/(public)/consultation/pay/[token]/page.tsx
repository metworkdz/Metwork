...
export const metadata: Metadata = {
  title: 'Consultation payment — Metwork',
...
export default async function ConsultationPayPage({ params }: PageProps) {
            ...
            <Card className="border-border/60">
              <CardContent className="p-6 sm:p-8">
                ...
                <dl className="mt-6 divide-y divide-border/60 overflow-hidden rounded-lg border border-border/60 text-sm">
                  {view.mentor && (
                    <Row label={t('consultant')} value={view.mentor.fullName} />
                  )}
                  {fmtDate(view.booking.scheduledAt) && (
                    <Row label={t('date')} value={fmtDate(view.booking.scheduledAt)!} />
                  )}
                  {view.booking.durationMinutes && (
                    <Row label={t('duration')} value={`${view.booking.durationMinutes} ${t('minutes')}`} />
                  )}
                  <Row
                    label={t('format')}
                    value={
                      view.booking.meetLink
                        ? t('online')
                        : view.booking.isOffline
                        ? t('inPerson')
                        : '—'
                    }
                  />
                  <div className="flex items-center justify-between bg-emerald-50/60 px-4 py-3">
                    ...
                    <dd className="text-base font-bold text-emerald-800">{fmtAmount(view.amount ?? 0)}</dd>
                  </div>
                </dl>

                <div className="mt-6">
                  <GuestPayButton token={token} />
                </div>
